...
namespace FS.PaymentService.Tspg
{
    ...
    {
        private const string PaymentGateway = "Tspg";
        ...
        public async Task HandleEventAsync(CallBackEto eventData)
        {
            ...
            if (eventData.Parameters.IsValid)
            {
                var eto = new PaymentSuccessEto()
                {
                    PaymentGateway = PaymentGateway,
                    PaymentId = paymentId,
                    PaymentNo = eventData.Parameters.OrderNo
                };
            ...
            {
                var eto = new PaymentFailedEto()
                {
                    PaymentGateway = PaymentGateway,
                    PaymentId = paymentId,
                    PaymentNo = eventData.Parameters.OrderNo,
                    Message = eventData.Parameters.ReturnMessage,
                    Code = eventData.Parameters.ReturnCode
                };
